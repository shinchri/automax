...
from users.forms import LocationForm, ProfileForm, UserForm
...
class RegisterView(View):
  ...
  def post(self, request):
    ...
    if register_form.is_valid():
      user = register_form.save() # saves user to the database
...
class ProfileView(View):
  ...
  def get(self, request):
    user_listings = Listing.objects.filter(seller=request.user.profile).all()
  ...
  def post(self, request):
    user_listings = Listing.objects.filter(seller=request.user.profile).all()
    user_liked_listings = LikedListing.objects.filter(profile=request.user.profile).all()
    user_form = UserForm(request.POST, instance=request.user) # without instance specified, it will create new instance
